...
const UserManagement = () => {
  ...
  const user = useSelector(selectUser);  const [pagination, setPagination] = useState({
    current: 1,
    pageSize: 10,
    total: 0,
  });
    ...
    try {
      ...
      if (response && response.status) {
        ...
        setStudentPagination({
          ...
          total: metaData.total || studentData.length,
        });

        // Update statistics with student count
        setStatistics((prev) => ({
          ...prev,
          totalStudents: metaData.total || studentData.length,
        }));
      } else {
        message.error("Không thể tải danh sách học sinh");
        setStudents([]);
        // Reset student count on error
        setStatistics((prev) => ({
          ...prev,
          totalStudents: 0,
        }));
  ...
  const calculateStatistics = useCallback(
    (userData) => {
      const stats = {
        ...
        parents: userData.filter(
          (user) => user.role === "parent" || user.role === "user"
        ).length,
        admins: userData.filter((user) => user.role === "admin").length,
      };
      setStatistics((prev) => ({
        ...stats,
        totalStudents: prev.totalStudents, // Preserve existing student count
      }));
    },
    []
  );const fetchAllUsers = useCallback(async (page = 1, pageSize = 10) => {
    try {
      setLoading(true);
  ...
  useEffect(() => {
    applyFilters();
    applyStudentFilters();
  }, [applyFilters, applyStudentFilters]);  const handleCardClick = (filterType) => {
  ...
  // Handle pagination change for users
  const handleUserTableChange = (paginationConfig) => {
    const { current, pageSize } = paginationConfig;
    fetchAllUsers(current, pageSize);
  };

  // Handle pagination change for students
  const handleStudentTableChange = (paginationConfig) => {
    const { current, pageSize } = paginationConfig;
    fetchAllStudents(current, pageSize);
  };
  ...
  }; // Card data for filtering - use useMemo to ensure it updates when statistics change
  ...
  const columns = [
    ...
    },
    {
      ...
      onFilter: (value, record) =>
        record.role === value || (value === "parent" && record.role === "user"),
    },
    {
      title: "Thao tác",
      key: "action",
  ...
  const studentColumns = [
    ...
    },
    {
      ...
      dataIndex: "dob",
      key: "dob",
      render: (date) => (date ? date : "Chưa cập nhật"),
    },
    {
      title: "Địa chỉ",
      dataIndex: "address",
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="text-gray-500">
              {activeTab === "users" ? (
                ...
                <>
                  Hiển thị {filteredStudents.length} / {students.length} học
                  sinh
                </>
              )}
            </div>
      ...
      <Card>        {activeTab === "users" ? (
          <Table
            ...
            pagination={{
              ...
              total: searchText || activeFilter !== 'all' ? filteredUsers.length : pagination.total,
          ...
          <Table
            ...
            pagination={{
              ...
              total: searchText ? filteredStudents.length : studentPagination.total,
              showSizeChanger: true,
              showQuickJumper: true,
              showTotal: (total, range) =>
        ...
        <div>
          ...
          >
            ...
            <ul style={{ margin: 0, paddingLeft: 16, color: "#666" }}>
              <li>
                File Excel phải chứa các cột: fullName, email, phone, password,
                role
              </li>
          ...
          <div>
            ...
            <Descriptions bordered column={2} size="small">
              ...
              <Descriptions.Item label="Lớp" span={1}>
                <Tag color="blue">
                  {selectedStudent.class || "Chưa cập nhật"}
                </Tag>
              </Descriptions.Item>
              <Descriptions.Item label="Giới tính" span={1}>
